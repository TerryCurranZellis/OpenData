...
/** Template plugin showing the expected OpenData lifecycle. */
public final class ExamplePlugin implements OpenDataPlugin {
    ...
    /** Constructor used by ReflectionPluginFactory. */
    public ExamplePlugin(final PluginDefinition definition) {
        this(ExamplePluginConfiguration.from(definition), new ExampleSourceClient(), new ExampleTransformer());
    }
    ...
    public PluginMetrics execute(final PluginExecutionContext context) throws Exception {
        ...
        final List<ExampleRecord> records = transformer.transform(payload);

        if (context.dryRun()) {
            return new PluginMetrics(records.size(), 0, 0, records.size());
        }

        final ExampleRepository.SaveResult result = new ExampleRepository(context.database())
                .save(records, context.runId());
        return new PluginMetrics(records.size(), result.inserted(), result.updated(), result.skipped());
    }
}
