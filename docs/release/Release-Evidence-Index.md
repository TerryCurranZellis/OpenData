...
**Candidate:** OpenData 2.0.0  
**Commit:** `<commit-sha>`  
**Reviewer:** `<name>`  
**Decision date:** `<date>`

| Gate | Evidence location | Result | Reviewer/notes |
|---|---|---|---|
| Clean source/version | `<path-or-link>` | Not run | |
| Maven build and tests | `<path-or-link>` | Not run | |
...
| ID | Blocker | Risk owner | Compensating control | Review/expiry | Approval |
|---|---|---|---|---|---|
| `<id>` | `<description>` | `<owner>` | `<control>` | `<date>` | `<decision>` |

An empty evidence location is not a pass. Keep sensitive logs and configuration in
restricted release records, not in public archives.
