...
| Medium | SQL installation ordering has been superseded by the consolidated `/sql` folder in version 2 | Historical split between `001-core-plugin-run.sql` and `sqlserver/010-create-core-schema.sql` | Retain one ordered installation manifest or migration tool |
