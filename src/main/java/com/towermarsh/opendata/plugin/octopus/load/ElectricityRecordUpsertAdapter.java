...
/**
 * SQL Server bindings for Octopus electricity billing records.
 *
 * @author Terry Curran
 * @version 2.0.0
 * @since 2.0.0
 */
final class ElectricityRecordUpsertAdapter
        extends AbstractOctopusUpsertAdapter<ElectricityRecord> {

    private static final String EXISTS_SQL = """
            SELECT 1
              FROM octopus.electric_data
             WHERE bill_date = ?
    ...
    private static final String INSERT_SQL = """
            INSERT INTO octopus.electric_data
                (bill_date, bill_period_start, bill_period_end, tariff_name,
                 tariff_period_start, tariff_period_end, mpan, meter_id,
                 start_reading_date, start_reading_value, start_reading_type,
                 end_reading_date, end_reading_value, end_reading_type,
                 energy_used_kwh, unit_rate_p_kwh, standing_charge_rate_p_day,
                 standing_charge_total_gbp, total_cost_gbp, last_run_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """;

    private static final String UPDATE_SQL = """
            UPDATE octopus.electric_data
    ...
    /** Creates the electricity adapter. */
    ...
    protected int bindKey(
        ...
        statement.setString(parameter++, record.mpan());
