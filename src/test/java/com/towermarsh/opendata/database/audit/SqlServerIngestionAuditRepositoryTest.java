...
class SqlServerIngestionAuditRepositoryTest {
 ...
 @Test void rejectsUnknownDataset() throws Exception { when(statement.executeUpdate()).thenReturn(0); assertThrows(SQLException.class,()->repo.startRun("missing",null,"1.0")); }
 @Test void rejectsMissingGeneratedKey() throws Exception { when(statement.executeUpdate()).thenReturn(1); when(keys.next()).thenReturn(false); assertThrows(SQLException.class,()->repo.startRun("ofgem",null,"1.0")); }
 ...
 @Test void completeRejectsNonStartedRun() throws Exception { when(statement.executeUpdate()).thenReturn(0); var c=new IngestionRunCompletion(IngestionStatus.FAILED,0,0,0,Instant.now(),"bad"); assertThrows(SQLException.class,()->repo.completeRun(7,c)); }
